...
mod tests {
    ...
    fn injected_system_prompt_env_is_prepended_to_request_system() {
        let _lock = env_lock();
        std::env::set_var("CLAW_SYSTEM_PROMPT", "local persona");
        ...
        let payload = build_chat_completion_request(
            &MessageRequest {
                ...
                system: Some("tool instructions".to_string()),
                tools: None,
                tool_choice: None,
                stream: false,
            },
    ...
    #[test]
    fn injected_system_prompt_file_can_be_changed_without_rebuild() {
        let _lock = env_lock();
        ...
        std::env::set_var("CLAW_SYSTEM_PROMPT", "ignored direct persona");

        let payload_v1 = build_chat_completion_request(
            &MessageRequest {
                model: "gemma4".to_string(),
                max_tokens: 64,
                messages: vec![InputMessage::user_text("hello")],
        ...
        assert_eq!(
            payload_v1["messages"][0]["content"],
            json!("file persona v1")
        );

        fs::write(&path, "file persona v2").expect("write prompt v2");
        let payload_v2 = build_chat_completion_request(
            &MessageRequest {
                model: "gemma4".to_string(),
                max_tokens: 64,
                messages: vec![InputMessage::user_text("hello")],
