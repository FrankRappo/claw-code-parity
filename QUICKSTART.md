...
| `/agent on` | Для следующего пользовательского turn обязателен один реальный tool call |
| `/agent off` | Обычный режим обсуждения и `tool_choice=auto` |
| `/agent status` | Показать текущий режим выполнения |
